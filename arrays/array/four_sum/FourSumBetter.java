...
public class FourSumBetter {
	...
	public static List<List<Integer>> fourSum(int[] nums, int target) {
		...
		for (int firstLoop = 0; firstLoop < nums.length - 2; firstLoop++) {
			for (int secondLoop = firstLoop + 1; secondLoop < nums.length - 1; secondLoop++) {
				HashSet<Integer> hash = new HashSet<Integer>();
				for (int thirdLoop = secondLoop + 1; thirdLoop < nums.length; thirdLoop++) {
					int fourthNumber = target - (nums[firstLoop] + nums[secondLoop] + nums[thirdLoop]);
					if (hash.contains(fourthNumber)) {
						List<Integer> arrayList = new ArrayList<Integer>();
						arrayList.add(nums[firstLoop]);
						arrayList.add(nums[secondLoop]);
						arrayList.add(nums[thirdLoop]);
						arrayList.add(fourthNumber);
						Collections.sort(arrayList);
						hashSet.add(arrayList);
					}
					hash.add(nums[thirdLoop]);
				}
			}
		}
